...
public class StudentWS {
    ...
    public Student find(@WebParam(name = "id") String id) {
        EntityManager em = Persistence.createEntityManagerFactory("WSStudentPracticePU").createEntityManager();
        try{
            Query query = em.createNamedQuery("Studentlist.findByRollNo", Student.class).setParameter("id", id);
    ...
    public Student insert(@WebParam(name = "rollNo") String rollNo,@WebParam(name = "fullname") String fullname, 
            @WebParam(name = "gender") String gender, @WebParam(name = "email") String email, @WebParam(name = "birthday") Date birthday) {
        EntityManager em = Persistence.createEntityManagerFactory("WSStudentPracticePU").createEntityManager();
        try{
            EntityTransaction et = em.getTransaction();
            et.begin();
            Student student = new Student(Integer.parseInt(rollNo), fullname, Boolean.parseBoolean(gender), email, birthday);
            em.persist(student);
            et.commit();
            return student;
        }catch(Exception e){
    ...
    public Student update(@WebParam(name = "rollNo") String rollNo,@WebParam(name = "fullname") String fullname, 
        ...
        EntityManager em = Persistence.createEntityManagerFactory("WSStudentPracticePU").createEntityManager();
        try{
            Query query = em.createNamedQuery("Student.findByRollNo", Student.class).setParameter("rollNo", rollNo);
            Student student = (Student) query.getSingleResult();
            if(student != null){
                EntityTransaction et = em.getTransaction();
                et.begin();
    ...
    public String delete(@WebParam(name = "rollNo") String rollNo) {
         EntityManager em = Persistence.createEntityManagerFactory("WSStudentPracticePU").createEntityManager();
         Query query = em.createNamedQuery("Student.findByRollNo", Student.class).setParameter("id", Integer.parseInt(rollNo));
